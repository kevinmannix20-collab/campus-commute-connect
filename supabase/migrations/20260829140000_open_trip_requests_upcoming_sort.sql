-- Browse should read like a schedule, not an activity log: soonest trip
-- first, and a post drops out once its requested_time has passed rather
-- than lingering as a stale, un-joinable card. Same return type as
-- before, so CREATE OR REPLACE (no drop needed).
